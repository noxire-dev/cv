...
const ThemeCtx = createContext<AccentTheme>({ accent: LAVENDER, accentSoft: LAVENDER_SOFT })
function useTheme() { return useContext(ThemeCtx) }


// Design 49: MASTER — SYSTEM_052 LAVENDER v2
// Based on Design 47 with:
//   - Vertical project grid (not horizontal scroll)
//   - Dedicated PROJECTS list page (like LOG)
//   - Scroll-to-top on every view change
//   - Back buttons always → home
//   - Nav links: WORK / PROJECTS / LOG
//   - Scrolling left coordinate rail
//   - Grid aligned to coordinate indicators
//   - Readability bumps throughout

// ── Dev color picker (Ctrl+Shift+D) ──

const DEV_PRESETS = [
  { name: 'HAZE', hex: '#957FB8' },
  { name: 'LAVENDER', hex: '#CDB4DB' },
  { name: 'WISTERIA', hex: '#C9A0DC' },
  { name: 'DUSK', hex: '#A78BFA' },
  { name: 'MIST', hex: '#9092C3' },
  { name: 'IRIS', hex: '#A8ABEB' },
]
...
function DevColorPicker({ accent, setAccent }: { accent: string; setAccent: (c: string) => void }) {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[300] font-mono border p-3"
...
function ScanlineOverlay({ trigger }: { trigger: string }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ background: accent, boxShadow: `0 0 20px ${accent}, 0 0 60px ${accent}40` }}
        initial={{ top: '0%' }}
        animate={{ top: '100%' }}
...
function PanelHeader({ title, status }: { title: string; status?: string }) {
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2 border-b" style={{ borderColor: EDGE, background: PANEL_DARK }}>
      ...
      {status && <span className="text-[11px] font-mono" style={{ color: accent }}>{status}</span>}
    </div>
  )
}
        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-2">
              <span className="text-base font-black font-sans" style={{ color: BRIGHT }}>{job.role}</span>
              <span className="text-[12px]" style={{ color: accent }}>{job.tagline}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3">
              {job.highlights.slice(0, 3).map(h => (
                <span key={h.stat} className="text-[12px]">
                  <span className="font-black" style={{ color: accent }}>{h.stat}</span>
          ...
          <div className="border mb-8" style={{ borderColor: EDGE, background: PANEL }}>
            ...
            <div className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
                  <h1 className="text-3xl md:text-4xl font-black font-sans mb-1" style={{ color: BRIGHT }}>{job.role}</h1>
                  <p className="text-base" style={{ color: accent }}>{job.company} · {job.tagline}</p>
                </div>
...
function Nav({ currentView, navigate }: { currentView: View; navigate: (v: View) => void }) {
  ...
  const navItems: { view: View; label: string; matchAlso?: View[] }[] = [
    { view: 'home', label: 'WORK', matchAlso: ['experience'] },
    { view: 'projects', label: 'PROJECTS', matchAlso: ['project'] },
    { view: 'blog', label: 'LOG', matchAlso: ['blog-post'] },
  ]
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 h-11 flex items-center justify-between text-[11px]">
        <div className="flex items-center gap-6">
          <button onClick={() => navigate('home')} className="flex items-center gap-1.5">
            <span className="font-bold" style={{ color: accent }}>SYS</span>
        ...
        <div className="flex items-center gap-5">
          {navItems.map(item => {
            ...
            return (
              <button key={item.view} onClick={() => navigate(item.view)} className="tracking-wider transition-colors"
          ...
          <span className="tabular-nums hidden md:inline" style={{ color: `${accent}80` }}>{time.toLocaleTimeString('en-GB', { hour12: false })}</span>
      ...
      <div className="max-w-[900px] mx-auto px-4 md:px-8 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="border mb-6" style={{ borderColor: EDGE, background: PANEL }}>
            ...
            <div className="p-6">
              <h1 className="text-4xl md:text-5xl font-black font-sans mb-2" style={{ color: BRIGHT }}>{p.name}</h1>
              <p className="text-[15px] leading-relaxed" style={{ color: MID }}>{p.impact}</p>
            </div>
          </div>

          <div className="border mb-6 overflow-hidden" style={{ borderColor: EDGE }}>
            ...
            <div className="aspect-video flex items-center justify-center relative" style={{ background: PANEL_DARK }}>
              <span className="text-[120px] leading-none select-none" style={{ opacity: 0.08, color: accent }}>{p.glyph}</span>
              <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: `${accent}20` }} />
            </div>
          </div>
            ...
            <div className="p-4 space-y-2">
              {p.features.map((f, i) => (
                <div key={f} className="flex items-center gap-3 text-sm">
                  <span className="text-[11px]" style={{ color: accent }}>{String(i).padStart(2, '0')}</span>
          ...
          <a href={p.link} target="_blank" rel="noopener"
            className="inline-flex items-center gap-2 text-[12px] font-bold px-5 py-2.5 border transition-colors"
          ...
          {projects.map((project, i) => (
            <motion.button key={project.id}
              ...
              <div className="aspect-[16/9] flex items-center justify-center relative overflow-hidden" style={{ background: PANEL_DARK }}>
                <span className="text-[80px] leading-none select-none" style={{ opacity: 0.12, color: accent }}>{project.glyph}</span>
                <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: `${accent}30` }} />
                <div className="absolute top-3 left-3">
                  <span className="text-[11px] font-mono" style={{ color: `${accent}50` }}>{project.idx}</span>
                </div>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[12px]" style={{ color: accent }}>{project.idx}</span>
                ...
                <h3 className="text-xl font-black font-sans mb-1 transition-colors" style={{ color: BRIGHT }}
                ...
                <span className="text-[12px] font-bold transition-transform inline-block group-hover:translate-x-1" style={{ color: accent }}>OPEN_CASE →</span>
              </div>
            </motion.button>
          ))}
        </div>
      ...
      <div className="max-w-[680px] mx-auto px-4 md:px-8 py-10">
        ...
        <motion.article initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-4">
            ...
            {post.tags.map(t => (
              <span key={t} className="text-[11px] px-1.5 py-0.5" style={{ background: accent, color: BG }}>{t}</span>
            ))}
          </div>
          <h1 className="text-2xl md:text-3xl font-black font-sans mb-8" style={{ color: BRIGHT }}>{post.title}</h1>
    ...
    <section className="min-h-screen flex flex-col justify-center px-4 md:px-8 pt-14">
      <div className="max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-12 gap-x-4 mb-8">
          <motion.div className="col-span-12 md:col-span-9"
            ...
            <div className="flex items-baseline gap-4 mb-2">
              <Coord label="A0" />
              <h1 className="text-[18vw] md:text-[11vw] font-black font-sans leading-[0.82] tracking-tighter" style={{ color: BRIGHT }}>SINA</h1>
            </div>
            <div className="flex items-baseline gap-4">
              <Coord label="A1" />
              <h1 className="text-[18vw] md:text-[11vw] font-black font-sans leading-[0.82] tracking-tighter" style={{ color: accent }}>DILEK</h1>
            </div>
          </motion.div>
              ...
              <div className="p-3 space-y-3 text-[13px]">
                {[
                  ...
                  <div key={d.k} className="flex justify-between">
                    ...
                    <span style={{ color: d.k === 'SCORE' ? accent : BRIGHT }}>{d.v}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}
          ...
          <a href="/Sina_Dilek_CV.pdf" download className="px-5 py-2 font-bold tracking-wider border transition-colors"
            style={{ borderColor: accent, color: accent }}
            onMouseEnter={e => { e.currentTarget.style.background = accent; e.currentTarget.style.color = BG }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = accent }}>
            DOWNLOAD_CV ↓
          </a>
          <a href="mailto:hi@sinadilek.com" className="px-5 py-2 font-bold tracking-wider border" style={{ borderColor: EDGE, color: MID }}>
            CONTACT
          </a>
          ...
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <section className="px-4 md:px-8 py-16">
              <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-4">
                <div className="col-span-12 md:col-span-3">
                  <Coord label="B0" />
                  <h2 className="text-2xl font-black font-sans mt-2 mb-1">EXPERIENCE</h2>
            ...
            <section className="px-4 md:px-8 py-16">
              <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-4">
                <div className="col-span-12 md:col-span-3">
                  <Coord label="C0" />
                  <h2 className="text-2xl font-black font-sans mt-2 mb-1">STACK</h2>
                ...
                <div className="col-span-12 md:col-span-9 space-y-4">
                  {(['LANGUAGES', 'FRAMEWORKS', 'SYSTEMS', 'TOOLS'] as const).map(category => {
                    ...
                    return (
                      <div key={category} className="border" style={{ borderColor: EDGE, background: PANEL }}>
                            ...
                            <motion.div key={cap.name} className="group flex items-center gap-2"
                              ...
                              <div className="flex-1 h-3.5 relative" style={{ background: PANEL_DARK }}>
                                <motion.div className="absolute inset-y-0 left-0"
                                  ...
                                  style={{ background: `${accent}${cap.level === 'PRIMARY' ? 'CC' : cap.level === 'FRAMEWORK' ? '80' : '50'}` }} />
            ...
            <section className="px-4 md:px-8 py-16">
              <div className="max-w-[1400px] mx-auto">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <Coord label="D0" />
                    <h2 className="text-2xl font-black font-sans mt-2">WORK</h2>
                  </div>
                  <button onClick={() => navigate('projects')} className="text-[12px] font-bold" style={{ color: accent }}>
                  ...
                  {projects.slice(0, 3).map((project, i) => (
                    <motion.button key={project.id}
                      ...
                      <div className="aspect-[16/10] flex items-center justify-center relative overflow-hidden" style={{ background: PANEL_DARK }}>
                        <span className="text-[72px] leading-none select-none" style={{ opacity: 0.12, color: accent }}>{project.glyph}</span>
                        <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: `${accent}30` }} />
                        <div className="absolute top-3 left-3">
                          <span className="text-[11px] font-mono" style={{ color: `${accent}50` }}>{project.idx}</span>
                        </div>
                      ...
                      <div className="p-4">
                        ...
                        <h3 className="text-lg font-black font-sans mb-1 transition-colors group-hover:!text-[var(--accent)]" style={{ color: BRIGHT }}>
                          {project.name}
                        </h3>
            ...
            <section className="px-4 md:px-8 py-16">
              <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-4">
                <div className="col-span-12 md:col-span-3">
                  <Coord label="E0" />
                  <h2 className="text-2xl font-black font-sans mt-2 mb-1">LOG</h2>
                  <button onClick={() => navigate('blog')} className="text-[12px] font-bold" style={{ color: accent }}>VIEW_ALL →</button>
                </div>
            ...
            <section className="px-4 md:px-8 py-20">
              <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-4">
                <div className="col-span-12 md:col-span-6">
                  <Coord label="F0" />
                  <h2 className="text-4xl md:text-6xl font-black font-sans mt-2 leading-tight">
                ...
                <div className="col-span-12 md:col-span-6 flex items-end">
                  <div className="w-full space-y-3">
                    ...
                    <motion.a href="mailto:hi@sinadilek.com"
                      ...
                      onMouseLeave={e => { e.currentTarget.style.background = `${accent}10`; e.currentTarget.querySelector<HTMLElement>('[data-cta-label]')!.style.color = accent; e.currentTarget.querySelector<HTMLElement>('[data-cta-value]')!.style.color = BRIGHT; e.currentTarget.querySelector<HTMLElement>('[data-cta-arrow]')!.style.color = accent }}>
                      <div>
                        <span data-cta-label className="text-[12px] tracking-[0.18em] font-bold block mb-0.5" style={{ color: accent }}>GET IN TOUCH</span>
            ...
            <footer className="px-4 md:px-8 py-4 border-t" style={{ borderColor: EDGE }}>
              <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-center gap-2 text-[11px]" style={{ color: DIM }}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span style={{ color: `${accent}60` }}>DESIGNED & BUILT FROM SCRATCH</span>
                </div>
              </div>
            </footer>
          </motion.div>
